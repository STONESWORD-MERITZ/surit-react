...
| 우선순위 | 항목 | 예상 | 비고 |
|---|---|---|---|
| P0 | 동일 자료 결과 결정성 보장 | ~1~2시간 | 같은 PDF/비밀번호로 100회 실행해도 고지 대상 질병코드·질병명·건수·질문 분류가 항상 동일해야 함 |
...
**동일 자료 결과 결정성 보장**을 먼저 권장합니다.

현재 사용자 관찰상 같은 자료를 반복 분석할 때 고지사항이 달라질 수 있다. 목표는 아래와 같이 분리한다.

- 고정되어야 하는 값: 고지 대상 질병코드, 질병명, 건수, 질문 분류(Q1~Q4), 입원/수술/통원/투약 같은 결정론 근거
- 변경 가능성이 허용되는 값: AI 판단이 개입되는 추가검사/재검사 의심 소견 문장 또는 보조 판단 설명
- 검증 방식: 동일 PDF 3종(세부진료내역, 처방전, 진료기록지)을 같은 비밀번호로 반복 실행하고, AI 보조 설명을 제외한 결정론 결과 subset을 비교
- 구현 방향: 파싱 결과 정렬, set/list 직렬화 순서 고정, disease grouping/report builder 출력 정렬, AI 결과가 결정론 고지 건수·코드·질병명에 영향을 주지 않는 경계 확인

사용자가 제공한 PDF 비밀번호는 민감정보 성격이 있어 저장소 문서에 직접 기록하지 않는다. 실제 검증 시 현재 대화에서 제공된 값을 사용한다.

그다음은 **실제 PDF 배포 후 재테스트**를 권장합니다.
