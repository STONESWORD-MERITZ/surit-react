...
def _open_pdf(data, bdate_str):
    ...
    if bd:
        ...
        if len(bd) == 8:
            candidates.append(bd[2:])
        elif len(bd) == 6:
            prefix = "20" if int(bd[:2]) <= 24 else "19"
            candidates.append(prefix + bd)
...
def _make_merged_item(item: dict, q: str) -> dict:
    return {
        "dates":          [item.get("date", "")],
        "code":           item.get("code", "-"),
...
def run_analysis(active_files, product_type, reference_date, birthdate_pw, api_key) -> dict:
    ...
    for uploaded_file in active_files:
        file_recs = []
        try:
            with _open_pdf(uploaded_file.read(), birthdate_pw or "") as pdf:
        ...
        for q in q_list:
            ...
            if source == "code":
                ...
                if merge_key not in merged_items:
                    merged_items[merge_key] = _make_merged_item(item, q)
                continue
